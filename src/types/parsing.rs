...
///
/// // collects the data from the capture groups of the regex into a tuple
/// fn collect_data(input: &mut Input, captures: &Captures) -> Result<(u32, u32), ParseError> {
///     // convert the capture groups to actual numbers
///     let number1 = captures.name("number1").ok_or(ParseError)?
///                           .as_str().parse().map_err(|_| ParseError)?;
///     let number2_match = captures.name("number2").ok_or(ParseError)?;
///     let number2 = number2_match.as_str().parse().map_err(|_| ParseError)?;
///
///     // move the cursor to after the second number
///     input.cursor_mut().forward(number2_match.end());
///
///     Ok((number1, number2))
/// }
///
/// // creates the actual result type (would actually create an Object or Element that needs
/// // SharedBehaviorData)
/// fn construct_result(
///     (number1, number2): (u32, u32), // the result of `collect_data`
///     _sbd: SharedBehaviorData, // the shared behavior data generated by `do_parse`
/// ) -> Result<u32, ()> {
